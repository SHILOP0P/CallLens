...
func (r *Repository) ListAdminCompanies(ctx context.Context, input models.ListAdminCompaniesInput) (models.ListAdminCompaniesResult, error) {
	...
	rows, err := r.db.QueryContext(ctx, fmt.Sprintf("SELECT company_uuid,name,owner_user_uuid,created_at,COUNT(*) OVER() FROM companies WHERE %s ORDER BY created_at DESC LIMIT $%d OFFSET $%d", where, len(args)-1, len(args)), args...)
	if err != nil {
...
func (r *Repository) GetAdminCompanyByUUID(ctx context.Context, id uuid.UUID) (models.AdminCompany, error) {
	var c models.AdminCompany
	err := r.db.QueryRowContext(ctx, "SELECT company_uuid,name,owner_user_uuid,created_at FROM companies WHERE company_uuid=$1 AND deleted_at IS NULL", id).Scan(&c.ID, &c.Name, &c.ManagerUserUUID, &c.CreatedAt)
...
func getAdminCompany(ctx context.Context, q queryRower, id uuid.UUID) (models.AdminCompany, error) {
	var c models.AdminCompany
	err := q.QueryRowContext(ctx, "SELECT company_uuid,name,owner_user_uuid,created_at FROM companies WHERE company_uuid=$1 AND deleted_at IS NULL", id).Scan(&c.ID, &c.Name, &c.ManagerUserUUID, &c.CreatedAt)
	if errors.Is(err, sql.ErrNoRows) {
		return c, models.ErrCompanyNotFound
	}
	return c, err
